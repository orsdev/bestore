...
export class ShoppingCart {
  ...
  static addItemToCart = ({ itemToBeAdded, cartItems }: AddItemProps) => {
    ...
    const existingItem = cartItems.find(
      (item) => item.product_id === itemToBeAdded.product_id,
    );
  ...
  static removeItemFromCart = (
    ...
    const newCartItems = cartItems.filter(
      (item) => item.product_id !== itemToBeClearId,
    );
    return newCartItems;
  };
  ...
  static reduceItemQty = (itemId: number, cartItems: ICart[]) => {
    ...
    const existingItem = cartItems.find((item) => item.product_id === itemId);
    let newCartItems;
   ...
   */
  static calculateSubTotal = (items: ICart[]) => {
    return items?.reduce((accumulator, item) => {
      const sum =
        item.quantity *
        getDiscountedPrice(item.price as number, item.discount as number);
